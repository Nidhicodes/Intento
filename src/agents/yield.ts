/**
 * Yield Scanner Agent — gathers APY evidence via iterative x402 data buys.
 *
 * Same costly-signaling loop as the Risk Agent: it buys protocol APY data one
 * point at a time, self-assesses coverage, and keeps spending (capped onchain by
 * its 20 USDC ERC-7710 delegation) until its picture is complete or budget runs out.
 * The USDC it commits to evidence is its trust weight in the orchestrator's decision.
 */
import { VENICE_API_URL, VENICE_MODEL } from '../config.js';
import type { AgentOutput, YieldResult, Portfolio, PermissionSpec } from '../types.js';
...
Given the portfolio plus evidence gathered so far, return ONLY valid JSON:
{
  ...
  "dataSufficiency": <0.0-1.0, how completely you've scanned the relevant protocols>,
...
export async function runYieldAgent(
  ...
  spec: PermissionSpec,
  apiKey: string
): Promise<AgentOutput<YieldResult>> {
  ...
  return {
    ...
    usdcSpent,
    budget: BUDGET_USDC,
    confidence: usdcSpent / BUDGET_USDC,
...
async function callVenice(
  ...
  evidence: unknown[],
  apiKey: string
): Promise<YieldResult & { dataSufficiency?: number; needMoreData?: boolean }> {
  ...
  try {
    ...
    return { opportunities: [], recommendedMove: null, dataSufficiency: 0.5, needMoreData: false };
  }
}
